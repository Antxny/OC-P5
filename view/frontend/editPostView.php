    ...
    <form action="index.php?action=editPost&id=<?= $post['id'] ?>" method="POST" enctype="multipart/form-data" class="col-md-6 mx-auto">
        ...
        <div class="row">
            <div class="form-group col-lg-12">
                ...
                <input type="text" class="form-control" name="author" id="form-post-author" value="<?= $post['author'] ?>" required>
            </div>
        </div>
        <div class="row">
            <div class="form-group col-lg-12">
                <label for="form-post-title">Titre de l'article*</label>
                <input type="text" class="form-control" name="title" id="form-post-title" value="<?= $post['title'] ?>" required>
            </div>
        </div>
        <div class="form-group">
            <label for="form-post-content">Contenu de l'article*</label>
            <textarea class="form-control" name="content" id="form-post-content" required><?= $post['content'] ?></textarea>
        </div>
        <div class="form-group">
